...
// Array
short   reg[8];     // general-purpose register
short   rom[256];   // program region (8bit)
short   ram[256];   // data retion (8bit)

// Prototype Declarement
void assembler(void);
...
int main(int argc, char **argv)
{
    ...
    short flag_eq;  // flag for comparison
    ...
    do {
        ...
        switch (op_code(ir)) {
            ...
            case SRA:
                reg[op_regA(ir)] >>= 1;
                reg[op_regA(ir)] |= ((reg[op_regA(ir)] & 0x4000) << 1);
                break;
            case LDL:
                reg[op_regA(ir)] &= 0xff00;
                reg[op_regA(ir)] |= op_data(ir);
                break;
            case LDH:
                reg[op_regA(ir)] &= 0x00ff;
                reg[op_regA(ir)] |= (op_data(ir) << 8);
                break;
            case CMP:
                if (reg[op_regA(ir)] == reg[op_regB(ir)]) flag_eq = 1;
                else flag_eq = 0;
